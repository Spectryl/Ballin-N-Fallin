...
public partial class Level : Node2D {
    ...
    public override void _Ready(){
        ...
        if(background != null){
            ...
            levelBackground = GD.Load<PackedScene>("res://Scenes/Object Scenes/Backgrounds/BackgroundTemplate.tscn").Instantiate<LevelBackground>();
        }
        ...
        if(Online.IsOnline){ //If not host spawn this immediately to prevent errors from not having the node when rpcs come
            ...
            Mode.ModeNode.AddChild(GD.Load<PackedScene>("res://Scenes/Object Scenes/Players/PlayerSynchronizer.tscn").Instantiate());
        }
    }
    ...
    private void SpawnPlayers(Vector2[] playerSpawns,byte flippedStart,string[] teams){
        ...
        for(int i = 0; i < Game.TotalPlayers; i++){
            Player player = GD.Load<PackedScene>("res://Scenes/Object Scenes/Players/Player.tscn").Instantiate<Player>();
            player.Id = (byte)(i+1);
