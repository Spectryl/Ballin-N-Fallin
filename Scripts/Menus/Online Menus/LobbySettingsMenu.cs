...
public partial class LobbySettingsMenu : VerticalMenu, ILeftRightSelections{
	...
	public override void _Process(double delta){
		...
		void createColorMenu(){
			InColorMenu = true;
			AddChild(GD.Load<PackedScene>("res://Scenes/Object Scenes/Players/ColorMenu.tscn").Instantiate());
		}
		...
		if(Online.IsHost()){
			switch(choice){
				case 5:
					...
					GetParent().GetParent().AddChild(GD.Load<PackedScene>("res://Scenes/Object Scenes/Menus/ModeMenu.tscn").Instantiate());		
					break;
				case 7:
					if(Tour.IsTour){
						Tour.PrepareTour();
